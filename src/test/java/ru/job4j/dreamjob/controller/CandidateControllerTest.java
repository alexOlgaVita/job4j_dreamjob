...
class CandidateControllerTest {
    ...
    @Test
    public void whenRequestCandidateByIdPageThenGetPageWithCities() {
        /* наверное, можно все проверить одним тестом выше, а не декомпозировать проверку и дублировать код */
        var candidate = new Candidate(1, "test1", "desc1", now(), 1, 2);
        when(candidateService.findById(candidate.getId())).thenReturn(Optional.of(candidate));
        var city1 = new City(1, "Москва");
        var city2 = new City(2, "Санкт-Петербург");
    ...
    public void whenPostUpdateCandidateWithFileThenSameDataAndRedirectToCandidatesPage() throws Exception {
        var candidateOld = new Candidate(1, "test0", "desc1", now(), 1, 2);
        when(candidateService.findById(candidateOld.getId())).thenReturn(Optional.of(candidateOld));
    ...
    @Test
    public void whenRequestDeleteCandidateByIdPageThenDeleteAndRedirectPageWithCandidates() {
        var candidate = new Candidate(1, "test1", "desc1", now(), 1, 2);
        when(candidateService.findById(candidate.getId())).thenReturn(Optional.of(candidate));
